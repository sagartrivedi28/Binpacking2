...
%         csv=importdata(handles.xlspath);
%         csv_data=[csv.data];
%         csv_text=[csv.textdata];
%         Dice_data1=cell(size(csv_data,1),9);
%         for i=1:size(csv_data,1)
%              Dice_data1{i,1}=csv_text{i,2};
%              for j=2:9
%                   Dice_data1{i,j}=csv_data(i,j-1);
%              end
%         end
...
xlswrite(handles.xlspathout,{'Dice No' 'Dice ID' 'Xsize' 'Ysize' 'Dice Xcor' 'Dice Ycor' 'Set ID' 'Dice-Yield' 'Volume'},1,'A1:I1');
xlswrite(handles.xlspathout,Fid,1,['A2:A' num2str(length(Fid)+1)]);
xlswrite(handles.xlspathout,FDice_data(:,1),1,['B2:B' num2str(length(Fid)+1)]);
xlswrite(handles.xlspathout,round(FDdim/handles.gsize)*handles.gsize,1,['C2:D' num2str(length(Fid)+1)]);
xlswrite(handles.xlspathout,round(Lcor/handles.gsize)*handles.gsize,1,['E2:F' num2str(length(Fid)+1)]);
xlswrite(handles.xlspathout,XLset,1,['G2:G' num2str(length(Fid)+1)]);
xlswrite(handles.xlspathout,MDPW,1,['H2:H' num2str(length(Fid)+1)]);
xlswrite(handles.xlspathout,Fvol,1,['I2:I' num2str(length(Fid)+1)]);

% Wafer Count: Calculation need to refine
xlswrite(handles.xlspathout,{'Set No' 'Dice No ' 'Wafer/Set'},1,'L1:N1');
xlswrite(handles.xlspathout,(1:length(Fsets))',1,['L2:L' num2str(length(Fsets)+1)]);
xlswrite(handles.xlspathout,XLFsets,1,['M2:M' num2str(length(Fsets)+1)]);
xlswrite(handles.xlspathout,ceil(Fwafer/GDPW),1,['N2:N' num2str(length(Fsets)+1)]);
xlswrite(handles.xlspathout,{'Total Wafer =',sum(ceil(Fwafer/GDPW))},1,['M' num2str(length(Fsets)+2) ':N' num2str(length(Fsets)+2)]);
xlswrite(handles.xlspathout,{'GDPW =',GDPW},1,['M' num2str(length(Fsets)+3) ':N' num2str(length(Fsets)+3)]);

% Myxlwrite(handles.xlspathout,{'Dice No' 'Dice ID' 'Xsize' 'Ysize' 'Dice Xcor' 'Dice Ycor' 'Set ID' 'Dice-Yield' 'Volume'},1,'A1:I1');
% Myxlwrite(handles.xlspathout,Fid,1,['A2:A' num2str(length(Fid)+1)]);
% Myxlwrite(handles.xlspathout,FDice_data(:,1),1,['B2:B' num2str(length(Fid)+1)]);
% Myxlwrite(handles.xlspathout,round(FDdim/handles.gsize)*handles.gsize,1,['C2:D' num2str(length(Fid)+1)]);
% Myxlwrite(handles.xlspathout,round(Lcor/handles.gsize)*handles.gsize,1,['E2:F' num2str(length(Fid)+1)]);
% Myxlwrite(handles.xlspathout,XLset,1,['G2:G' num2str(length(Fid)+1)]);
% Myxlwrite(handles.xlspathout,MDPW,1,['H2:H' num2str(length(Fid)+1)]);
% Myxlwrite(handles.xlspathout,Fvol,1,['I2:I' num2str(length(Fid)+1)]);
% 
% % Wafer Count: Calculation need to refine
% Myxlwrite(handles.xlspathout,{'Set No' 'Dice No ' 'Wafer/Set'},1,'L1:N1');
% Myxlwrite(handles.xlspathout,(1:length(Fsets))',1,['L2:L' num2str(length(Fsets)+1)]);
% Myxlwrite(handles.xlspathout,XLFsets,1,['M2:M' num2str(length(Fsets)+1)]);
% Myxlwrite(handles.xlspathout,ceil(Fwafer/GDPW),1,['N2:N' num2str(length(Fsets)+1)]);
% Myxlwrite(handles.xlspathout,{'Total Wafer =',sum(ceil(Fwafer/GDPW))},1,['M' num2str(length(Fsets)+2) ':N' num2str(length(Fsets)+2)]);
% Myxlwrite(handles.xlspathout,{'GDPW =',GDPW},1,['M' num2str(length(Fsets)+3) ':N' num2str(length(Fsets)+3)]);


% handles.xlspathout1 = fullfile(handles.pname,[handles.fname '_out1.txt']);
% mat_str_cat='Dice No, Dice ID, Dice Xcor, Dice Ycor, Set ID, Dice-Yield, Volume\r\n';
% mat_str=[];
% for i=1:size(Fid,1)
% mat_str=[mat_str num2str(Fid(i)) ',' FDice_data{i,1} ','];
%      for j=1:size(FDice_data,2)
%            mat_str=[mat_str num2str(FDice_data{i,j}) ',']; 
%      end
%      mat_str=[mat_str ',' num2str(Lcor(i,1)) ',' num2str(Lcor(i,2)) ',' regexprep(XLset{i},'\s+',' ') ',' num2str(MDPW(i)) ',' num2str(Fvol(i)) '\r\n'];
% end
% mat_str=[mat_str_cat mat_str];
% fid=fopen(handles.xlspathout1,'w');
% fprintf(fid,mat_str);
% fclose(fid);
% handles.xlspathout2 = fullfile(handles.pname,[handles.fname '_out2.txt']);
% mat_str_cat='Set No, Dice No, Wafer/Set\r\n';
% mat_str=[];
% for i=1:length(Fsets)
%      mat_str=[mat_str num2str(i) ',' regexprep(XLFsets{i},'\s+',' ') ',' num2str(ceil(Fwafer(i)/GDPW)) '\r\n'];
% end
% mat_str=[mat_str_cat mat_str];
% fid=fopen(handles.xlspathout2,'w');
% fprintf(fid,mat_str);
% fclose(fid);
